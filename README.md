...
> **项目已在团队实车上完成整车运动、四轮编码器反馈和闭环联调。当前默认分支为 `main`，仓库完整保留了四轮控制、运动映射与串口通信代码。**
...
项目完成后，整车由团队其他成员保管，当前公开材料以当时完成的代码和调试记录为准。`main` 中最后上传的运行入口保留在单轮闭环台架调试配置，方便逐轮检查方向、编码器反馈和 PID；这不影响项目已经完成的整车联调，但意味着重新部署时需要按文末说明恢复串口接收与四轮目标值输入，并结合具体车辆重新标定。
...
当前 `main` 代码快照的运行入口保留在左前轮单轮调试状态；`version2` 作为同一调试阶段的历史分支继续保留：
...
仓库当前提供四轮控制代码、通信协议与重新部署步骤；新的整车演示和速度跟踪数据将在团队素材可用后补充。项目已经完成的实车联调与当前代码快照的用途在本文中分别说明。
