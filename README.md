...
> **F401 底盘已在团队实车上完成四轮编码器反馈、闭环控制和直线运行测试。当前 `main` 默认以 100 RPM 启动四轮独立闭环直线模式。**
...
因此，直接编译烧录后会运行四轮闭环直线模式，串口指令暂时不会生效。首次上电仍应架空车轮检查方向，再放到地面低速测试。
...
仓库当前提供已验证的 F401 四轮直线闭环默认配置，以及尚待硬件链路修复后验证的双板通信代码。新的整车演示和速度跟踪数据将在团队素材可用后补充。
